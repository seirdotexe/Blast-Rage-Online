...
There appears to be some sort of bug going on when it comes to having a predefined server by default. The game does retrieve all servers but just never sets them. Instead it'll use **dev.mmocha.com** which is already set on `_SafeStr_519`. I suppose you're better off changing this variable to your IP. There's also a secret event somewhere that'll change the port to 1139.
